                ...
                <div className="space-y-4">
                    {schedules[selectedDate.toDateString()]?.length > 0 ? (
                        schedules[selectedDate.toDateString()].map((schedule: any, index: number) => (
                            <div
                            key={index}
                            className="p-6 rounded-2xl  border border-gray-200 bg-white w-full"
                        >
                            <div
    className={`text-center py-2 rounded-t-2xl font-semibold text-xs md:text-sm ${
        schedule.approval === "CONFIRMED"
            ? "bg-green-100 text-green-600"
            : schedule.approval === "COMPLETED"
...
>
    {schedule.approval === "PENDING"
        ...
        : "Confirmed"}
                            ...
                            <div className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-6">
